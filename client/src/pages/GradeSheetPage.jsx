...
//     // Simple custom sort if needed, or default alphabetic
//     return Array.from(classes).sort();
//   }, [allSubjects]);

//   // Get subjects filtered by the selected class
...
//   // Reset sheet when assessment changes
//   const handleAssessmentChange = (e) => {
//     setSelectedAssessment(e.target.value);
//     setSheetData(null);
//   };

//   // Load the actual grade entry sheet
...
//                 Maximum Marks: <span className="text-gray-800 bg-gray-200 px-2 py-0.5 rounded">{sheetData.assessmentType.totalMarks}</span>
//               </p>
//             </div>
//             <button
//               onClick={handleSave}
...
//                               ${scores[student._id] !== "" && scores[student._id] !== undefined 
...
const GradeSheetPage = () => {
  ...
  // 🌟 FIX: Send academicYear along with assessment ID to backend
  const handleLoadSheet = async () => {
    if (!selectedAssessment || !selectedSubject || !academicYear) return;
    setLoading(true);
    setError(null);
    try {
      // NOTE: gradeService.getGradeSheet function should accept an object or multiple params depending on how you've defined it in your service. 
      // If your service currently only takes an ID, update it to take year too.
      // Example: axios.get(`/api/grades/sheet?assessmentTypeId=${id}&academicYear=${year}`)
      ...
      const initialScores = {};
      response.data.students.forEach((s) => {
        initialScores[s._id] = s.score ?? ""; // It will automatically be blank if no marks exist for this session
      });
      setScores(initialScores);
    } catch (err) {
      setError(err.response?.data?.message || "Failed to load grade sheet.");
