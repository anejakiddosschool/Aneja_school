...
const GradeSheetPage = () => {
  ...
  // Create dynamic years list (2 past sessions, current session, and 2 future sessions)
  // For 2026, it gives: ["2024-2025", "2025-2026", "2026-2027", "2027-2028", "2028-2029"]
  ...
  const handleYearChange = (e) => {
    setAcademicYear(e.target.value);
    setSheetData(null); // Clear table when Session changes, so user doesn't see old session marks
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const scoresPayload = Object.keys(scores)
        // Allow '0' (number or string) but ignore empty string "" or null/undefined
        .filter((studentId) => {
           const val = scores[studentId];
           return val !== "" && val !== null && val !== undefined;
        })
