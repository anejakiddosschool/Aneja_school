...
// NEW: Radix dialog + ReportCardPage
...
const StudentListPage = () => {
  ...
  useEffect(() => {
    const checkWhatsappStatus = async () => {
      try {
        const res = await fetch("http://localhost:5001/api/whatsapp/status");
  ...
  const sendCustomMessageToParents = async () => {
    ...
    const confirmed = window.confirm(
      `Send your custom message to all ${filteredStudents.length} parents?`
    );
    if (!confirmed) return;
    ...
    for (const student of filteredStudents) {
      try {
        ...
        const response = await fetch(
          "http://localhost:5001/api/whatsapp/send-custom-message",
          {
            method: "POST",
  ...
  // Send report card messages only
  const sendReportCardsToParents = async () => {
    ...
    const confirmed = window.confirm(
      `Send report card links to all ${filteredStudents.length} parents?`
    );
    if (!confirmed) return;
  ...
  // Send personal message to single student
  const sendPersonalMessage = async (student) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5001/api/whatsapp/send-personal-message",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
