...
const StudentListPage = () => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    return allStudents
      ...
      .filter((student) => {
        ...
        return (
          student.fullName.toLowerCase().includes(q) ||
          (student.studentId && student.studentId.toString().toLowerCase().includes(q))
        );
      })
      .sort((a, b) => a.fullName.localeCompare(b.fullName));
  }, [selectedGrade, allStudents, searchQuery]);
  ...
  const handleCheckboxChange = (id, checked) => {
    setSelectedStudentIds((prev) => checked ? [...prev, id] : prev.filter((x) => x !== id));
  };
  ...
  // Generic Broadcaster Logic
  const processBroadcast = async (reportType, labelName, payloadFn) => {
    const validStudents = filteredStudents.filter((s) => selectedStudentIds.includes(s._id));
    if (validStudents.length === 0) return toast.error("No students selected.");
    if (!window.confirm(`Send ${labelName} to ${validStudents.length} parents?`)) return;

    for (const student of validStudents) {
      try {
        updateStatus(student._id, `${labelName}: Sending...`);
        const payload = payloadFn(student);
        const route = reportType === "custom" ? "/whatsapp/send-custom-message" : "/whatsapp/send-report-links";
        const res = await fetch(`${API_URL}${route}`, {
          method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload),
        });
        updateStatus(student._id, res.ok ? `${labelName}: Sent ✓` : `${labelName}: Failed ✗`);
      } catch {
        updateStatus(student._id, `${labelName}: Failed ✗`);
      }
    }
  ...
  const sendReportCardsToParents = () => {
    processBroadcast("reportCard", "Report Card", (student) => {
      const msg = `Dear ${student.parentContact?.parentName || "Parent"},\n\nGreetings from Aneja Kiddos School.\n\nWe are pleased to share the academic report card for your ward, *${student.fullName}*.\n\n*Student Details:*\n• Class: ${student.gradeLevel || "N/A"}\n• Section: ${student.section || "N/A"}\n\nPlease find the detailed report card attached with this message. We encourage you to review the academic progress and reach out to the class teacher for any queries.\n\nWarm regards,\n*Administration*\n*Aneja Kiddos School*`;
      return { students: [{ id: student._id, fullName: student.fullName, parentContact: student.parentContact, parentPhone: student.parentContact?.phone, reportLink: student.reportCardUrl, reportType: "reportCard", message: msg }] };
    });
  };
  ...
  const sendClassTestReportsToParents = () => {
    processBroadcast("classTest", "Class Test", (student) => {
      const msg = `Dear ${student.parentContact?.parentName || "Parent"},\n\nGreetings from Aneja Kiddos School.\n\nPlease find attached the recent *Class Test Report* for your ward.\n\n*Student Details:*\n• Name: *${student.fullName}*\n• Class: ${student.gradeLevel || "N/A"}\n• Section: ${student.section || "N/A"}\n\nWe encourage you to review the scores to track their continuous academic progress. If you have any concerns, please feel free to connect with the respective subject teachers.\n\nWarm regards,\n*Administration*\n*Aneja Kiddos School*`;
      return { students: [{ id: student._id, parentPhone: student.parentContact?.phone, reportLink: student.reportClassTestUrl, reportType: "classTest", message: msg }] };
    });
  };
  ...
  const sendNtseReportsToParents = () => {
    processBroadcast("ntse", "NTSE", (student) => {
      const msg = `Dear ${student.parentContact?.parentName || "Parent"},\n\nGreetings from Aneja Kiddos School.\n\nWe are sharing the attached *NTSE (National Talent Search Examination) Report* for your ward.\n\n*Student Details:*\n• Name: *${student.fullName}*\n• Class: ${student.gradeLevel || "N/A"}\n• Section: ${student.section || "N/A"}\n\nThis assessment helps us gauge their aptitude for competitive environments. Kindly go through the detailed report attached herewith.\n\nWarm regards,\n*Administration*\n*Aneja Kiddos School*`;
      return { students: [{ id: student._id, parentPhone: student.parentContact?.phone, reportLink: student.reportNTSEUrl, reportType: "ntse", message: msg }] };
    });
  };
  ...
  const sendPtReportsToParents = () => {
    processBroadcast("ptTest", "PT Test", (student) => {
      const msg = `Dear ${student.parentContact?.parentName || "Parent"},\n\nGreetings from Aneja Kiddos School.\n\nPlease find attached the *Periodic Test (PT) Report* for your ward.\n\n*Student Details:*\n• Name: *${student.fullName}*\n• Class: ${student.gradeLevel || "N/A"}\n• Section: ${student.section || "N/A"}\n\nPeriodic assessments are crucial for identifying areas of improvement and strengths. We request you to review the attached document and guide them accordingly at home.\n\nWarm regards,\n*Administration*\n*Aneja Kiddos School*`;
      return { students: [{ id: student._id, parentPhone: student.parentContact?.phone, reportLink: student.reportPTUrl, reportType: "ptTest", message: msg }] };
    });
  };

  const sendCustomBulk = () => {
    if (!customMessage.trim()) return toast.error("Please enter a custom message!");
    processBroadcast("custom", "Custom Msg", (student) => ({
      students: [{ id: student._id, parentPhone: student.parentContact?.phone, message: customMessage.trim() }],
    }));
  };
  ...
  const sendPersonalMsg = async (student) => {
    ...
    try {
      updateStatus(student._id, "Personal: Sending...");
      const payload = { students: [{ id: student._id, parentPhone: student.parentContact?.phone, message: personalMessageContent.trim() }] };
      const res = await fetch(`${API_URL}/whatsapp/send-personal-message`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });

      updateStatus(student._id, res.ok ? "Personal: Sent ✓" : "Personal: Failed ✗");
      if (res.ok) {
        setPersonalMessageStudentId(null);
        setPersonalMessageContent("");
        toast.success("Sent");
      ...
      <div className="max-w-[1300px] mx-auto space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100 gap-4">
          <div className="flex items-center justify-between w-full sm:w-auto gap-3">
            <h1 className="text-xl md:text-2xl font-extrabold text-gray-800">Student Directory</h1>
            <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider whitespace-nowrap ${whatsappReady ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
              {whatsappReady ? "● WA Online" : "● WA Offline"}
            </div>
          </div>
          ...
          <div className="flex flex-row w-full sm:w-auto gap-2">
            {currentUser.role === "admin" && (
              <>
                <Link to="/students/add" className="flex-1 sm:flex-none text-center bg-pink-600 hover:bg-pink-700 text-white text-sm font-bold py-2 md:py-1.5 px-4 rounded-lg transition-all">
                  + Add
                </Link>
                <Link to="/students/import" className="flex-1 sm:flex-none text-center bg-gray-800 hover:bg-gray-900 text-white text-sm font-bold py-2 md:py-1.5 px-4 rounded-lg transition-all">
                  📊 Import
                </Link>
              </>
            )}
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <span className="text-sm font-bold text-gray-500 uppercase whitespace-nowrap">Class:</span>
            <select
              value={selectedGrade}
              onChange={(e) => { setSelectedGrade(e.target.value); setSelectedStudentIds([]); }}
          ...
          <div className="bg-indigo-50 border border-indigo-100 p-3 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-fade-in">
            <div className="flex items-center justify-between w-full sm:w-auto">
                ...
                <label className="sm:hidden flex items-center gap-2 text-xs font-bold text-indigo-700">
                    <input type="checkbox" className="rounded text-pink-600 focus:ring-pink-500" onChange={(e) => handleSelectAll(e.target.checked)} checked={filteredStudents.length > 0 && filteredStudents.every((s) => selectedStudentIds.includes(s._id))} />
                    Select All
                </label>
            </div>

            <div className="flex flex-row overflow-x-auto w-full pb-2 sm:pb-0 gap-2 hide-scrollbar">
              <input type="text" placeholder="Type custom msg..." value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} className="min-w-[150px] border border-indigo-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-indigo-500 outline-none" />
              <button onClick={sendCustomBulk} disabled={!whatsappReady} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-bold px-4 py-2 rounded-lg transition whitespace-nowrap">💬 Send</button>
              <div className="w-px bg-indigo-200 shrink-0 mx-1 hidden sm:block"></div>
              <button onClick={sendReportCardsToParents} disabled={!whatsappReady} className="bg-pink-600 hover:bg-pink-700 disabled:opacity-50 text-white text-xs font-bold px-4 py-2 rounded-lg transition whitespace-nowrap">Report Card</button>
              <button onClick={sendClassTestReportsToParents} disabled={!whatsappReady} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold px-4 py-2 rounded-lg transition whitespace-nowrap">Class Test</button>
              <button onClick={sendNtseReportsToParents} disabled={!whatsappReady} className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white text-xs font-bold px-4 py-2 rounded-lg transition whitespace-nowrap">NTSE</button>
              <button onClick={sendPtReportsToParents} disabled={!whatsappReady} className="bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white text-xs font-bold px-4 py-2 rounded-lg transition whitespace-nowrap">PT</button>
            </div>
          </div>
        )}
        ...
        {selectedGrade ? (
          <>
            ...
            <div className="hidden md:block bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <thead>
                    ...
                    <th className="py-3 px-4 w-10 text-center">
                        <input type="checkbox" onChange={(e) => handleSelectAll(e.target.checked)} checked={filteredStudents.length > 0 && filteredStudents.every((s) => selectedStudentIds.includes(s._id))} className="cursor-pointer rounded text-pink-600 focus:ring-pink-500" />
                    </th>
                    <th className="py-3 px-4 font-bold text-gray-700">Student Detail</th>
                    <th className="py-3 px-4 font-bold text-gray-700 text-center">Attached Documents</th>
                    <th className="py-3 px-2 font-bold text-gray-700 text-center w-32">WA Status</th>
                    <th className="py-3 px-4 font-bold text-gray-700 text-right">Quick Action</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                    {filteredStudents.length > 0 ? (
                    filteredStudents.map((student) => {
                        ...
                        const hasNtse = !!student.reportNTSEUrl;
                        const hasPt = !!student.reportPTUrl;
                        const hasAnyDoc = hasReportCard || hasClassTest || hasNtse || hasPt;
                        const currentStatus = sendStatuses[student._id];

                        return (
                        <tr key={student._id} className="hover:bg-pink-50/50 transition-colors">
                            <td className="py-4 px-4 text-center">
                                <input type="checkbox" checked={selectedStudentIds.includes(student._id)} onChange={(e) => handleCheckboxChange(student._id, e.target.checked)} className="cursor-pointer rounded text-pink-600 focus:ring-pink-500 w-4 h-4" />
                            </td>
                            <td className="py-4 px-4">
                                <Link to={`/students/${student._id}`} className="font-bold text-gray-900 hover:text-pink-600 block text-[15px]">
                                    {student.fullName}
                                </Link>
                            ...
                            <td className="py-4 px-4 text-center">
                                <div className="flex flex-col gap-1 items-center justify-center bg-gray-50 border border-gray-100 rounded-lg p-2 min-w-[140px]">
                                    ...
                                    <>
                                        <div className="flex flex-wrap justify-center gap-1 w-full">
                                            {hasReportCard && <span className="bg-pink-100 text-pink-800 text-[10px] font-bold px-1.5 py-0.5 rounded border border-pink-200" title="Report Card">RC</span>}
                                            {hasClassTest && <span className="bg-blue-100 text-blue-800 text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200" title="Class Test">CT</span>}
                                            {hasNtse && <span className="bg-purple-100 text-purple-800 text-[10px] font-bold px-1.5 py-0.5 rounded border border-purple-200" title="NTSE Report">NTSE</span>}
                                            {hasPt && <span className="bg-teal-100 text-teal-800 text-[10px] font-bold px-1.5 py-0.5 rounded border border-teal-200" title="Periodic Test">PT</span>}
                                        </div>
                                        <span className="text-[10px] text-gray-500 flex items-center gap-1 font-medium mt-1">🗓️ {formatDate(student.updatedAt)}</span>
                                    </>
                                    ) : ( <span className="text-gray-400 text-[11px] italic py-1">No Docs Uploaded</span> )}
                                </div>
                            </td>
                            <td className="py-4 px-2 text-center">
                            {currentStatus && currentStatus !== "Idle" && (
                                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${currentStatus.includes("Sent") ? "bg-green-50 text-green-700 border-green-200" : currentStatus.includes("Failed") ? "bg-red-50 text-red-700 border-red-200" : "bg-yellow-50 text-yellow-700 border-yellow-200"}`}>
                                {currentStatus}
                                </span>
                            )}
                            </td>
                            ...
                            {personalMessageStudentId === student._id ? (
                                <div className="flex items-center gap-1 justify-end">
                                <input type="text" value={personalMessageContent} onChange={(e) => setPersonalMessageContent(e.target.value)} placeholder="Type..." className="text-[11px] border border-pink-300 rounded px-2 py-1.5 w-32 focus:outline-none focus:ring-1 focus:ring-pink-500" autoFocus />
                                <button onClick={() => sendPersonalMsg(student)} disabled={!personalMessageContent.trim() || !whatsappReady} className="bg-green-100 text-green-700 border border-green-200 rounded font-bold px-2 py-1 text-xs hover:bg-green-200">✓</button>
                                <button onClick={() => { setPersonalMessageStudentId(null); setPersonalMessageContent(""); }} className="bg-red-100 text-red-700 border border-red-200 rounded font-bold px-2 py-1 text-xs hover:bg-red-200">✕</button>
                                </div>
                            ) : (
                                <div className="flex justify-end gap-2">
                                <button onClick={() => setPersonalMessageStudentId(student._id)} className="text-gray-500 hover:text-blue-600 bg-gray-50 border border-gray-200 hover:border-blue-300 hover:bg-blue-50 p-2 rounded-lg transition shadow-sm" title="Message">💬</button>
                                <Dialog.Root open={reportStudentId === student._id} onOpenChange={(open) => setReportStudentId(open ? student._id : null)}>
                                    <Dialog.Trigger asChild>
                                    <button disabled={!hasReportCard} className={`p-2 rounded-lg transition shadow-sm border ${hasReportCard ? "text-gray-500 hover:text-green-600 bg-gray-50 border-gray-200 hover:border-green-300 hover:bg-green-50" : "text-gray-300 bg-gray-50 border-gray-100 cursor-not-allowed"}`} title="View Report Card">📄</button>
                                    </Dialog.Trigger>
                                    <Dialog.Portal>
                                    <Dialog.Overlay className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" />
                                    <Dialog.Content className="fixed inset-4 md:inset-10 bg-white rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col">
                                        <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                                        <Dialog.Title className="text-lg font-bold text-gray-800">Report Card: {student.fullName}</Dialog.Title>
                                        <Dialog.Close asChild><button className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-1.5 rounded-lg text-sm font-bold shadow-sm transition">✕ Close</button></Dialog.Close>
                                        </div>
                                        <div className="flex-1 overflow-y-auto p-4"><ReportCardPage studentId={student._id} /></div>
                                    </Dialog.Content>
                                    </Dialog.Portal>
                                </Dialog.Root>
                                </div>
                    ...
                    <tr><td colSpan={5} className="text-center py-20 text-gray-400 font-medium"><div className="text-4xl mb-3">📭</div>No students found.</td></tr>
                    )}
                </tbody>
                </table>
            </div>
            ...
            <div className="md:hidden flex flex-col gap-3">
                {filteredStudents.length > 0 ? (
                    filteredStudents.map((student) => {
                        ...
                        return (
                            <div key={student._id} className={`bg-white rounded-xl shadow-sm border p-4 transition-colors ${isSelected ? 'border-pink-400 bg-pink-50/20' : 'border-gray-200'}`}>
                                
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-start gap-3">
                                        <input type="checkbox" checked={isSelected} onChange={(e) => handleCheckboxChange(student._id, e.target.checked)} className="mt-1 w-5 h-5 cursor-pointer rounded border-gray-300 text-pink-600 focus:ring-pink-500" />
                                        <div>
                                            <Link to={`/students/${student._id}`} className="font-bold text-gray-900 text-[16px] leading-tight block">
                                                {student.fullName}
                                            </Link>
                                            <p className="text-xs text-gray-500 font-mono mt-0.5">ID: {student.studentId} • Roll: {student.rollNumber || "-"}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-gray-50 border border-gray-100 rounded-lg p-2.5 mb-3 flex items-center justify-between">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Docs Attached</span>
                                        {hasAnyDoc ? (
                                            <div className="flex gap-1">
                                                {hasReportCard && <span className="bg-pink-100 text-pink-800 text-[9px] font-bold px-1.5 py-0.5 rounded border border-pink-200">RC</span>}
                                                {hasClassTest && <span className="bg-blue-100 text-blue-800 text-[9px] font-bold px-1.5 py-0.5 rounded border border-blue-200">CT</span>}
                                                {hasNtse && <span className="bg-purple-100 text-purple-800 text-[9px] font-bold px-1.5 py-0.5 rounded border border-purple-200">NTSE</span>}
                                                {hasPt && <span className="bg-teal-100 text-teal-800 text-[9px] font-bold px-1.5 py-0.5 rounded border border-teal-200">PT</span>}
                                            </div>
                                        ) : (
                                    ...
                                    <div className="text-right">
                                         <span className="text-[9px] text-gray-400 block mb-0.5">Last Updated</span>
                                         <span className="text-[11px] font-medium text-gray-600">{formatDate(student.updatedAt)}</span>
                                    </div>
                                </div>

                                {currentStatus && currentStatus !== "Idle" && (
                                    <div className="mb-3">
                                        <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${currentStatus.includes("Sent") ? "bg-green-50 text-green-700" : currentStatus.includes("Failed") ? "bg-red-50 text-red-700" : "bg-yellow-50 text-yellow-700"}`}>
                                ...
                                <div className="flex gap-2 pt-2 border-t border-gray-100">
                                    <button onClick={() => setPersonalMessageStudentId(student._id)} className="flex-1 flex justify-center items-center gap-1 text-blue-600 bg-blue-50 hover:bg-blue-100 py-2 rounded-lg text-sm font-bold transition">💬 Msg</button>
                                    <Dialog.Root open={reportStudentId === student._id} onOpenChange={(open) => setReportStudentId(open ? student._id : null)}>
                                        <Dialog.Trigger asChild>
                                        <button disabled={!hasReportCard} className={`flex-1 flex justify-center items-center gap-1 py-2 rounded-lg text-sm font-bold transition ${hasReportCard ? "text-green-600 bg-green-50 hover:bg-green-100" : "text-gray-400 bg-gray-50 cursor-not-allowed"}`}>📄 Report</button>
                                        </Dialog.Trigger>
                                        <Dialog.Portal>
                                        <Dialog.Overlay className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" />
                                        <Dialog.Content className="fixed inset-2 bg-white rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col">
                                            <div className="p-3 bg-gray-50 border-b flex justify-between items-center">
                                            <Dialog.Title className="text-sm font-bold text-gray-800 truncate pr-2">Report: {student.fullName}</Dialog.Title>
                                            <Dialog.Close asChild><button className="bg-gray-200 text-gray-800 px-3 py-1.5 rounded-lg text-xs font-bold">✕ Close</button></Dialog.Close>
                                            </div>
                                            <div className="flex-1 overflow-y-auto p-2"><ReportCardPage studentId={student._id} /></div>
                                        </Dialog.Content>
                                        </Dialog.Portal>
                                    </Dialog.Root>
                                </div>
                                ...
                                {personalMessageStudentId === student._id && (
                                    <div className="mt-3 p-3 bg-blue-50 border border-blue-100 rounded-lg flex flex-col gap-2">
                                        <input type="text" value={personalMessageContent} onChange={(e) => setPersonalMessageContent(e.target.value)} placeholder="Type a message to parent..." className="w-full text-sm border border-blue-200 rounded p-2 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => { setPersonalMessageStudentId(null); setPersonalMessageContent(""); }} className="px-3 py-1.5 text-xs font-bold text-gray-500 bg-white border border-gray-200 rounded">Cancel</button>
                                            <button onClick={() => sendPersonalMsg(student)} disabled={!personalMessageContent.trim() || !whatsappReady} className="px-3 py-1.5 text-xs font-bold text-white bg-blue-600 rounded disabled:opacity-50">Send Message</button>
                                        </div>
                                    </div>
                                )}
                            </div>
          ...
          <div className="bg-white p-8 md:p-12 rounded-xl shadow-sm border border-gray-100 text-center flex flex-col items-center justify-center py-16 md:py-20">
            ...
            <p className="text-sm text-gray-500 mt-1 max-w-sm">
              Please select a class from the top dropdown to view students, check document status, and manage WhatsApp reporting.
            </p>
          </div>
        )}
