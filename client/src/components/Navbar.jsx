...
const Navbar = () => {
  ...
  const [whatsappStatus, setWhatsappStatus] = useState("Waiting for QR...");
  const navigate = useNavigate();
  const { notifications, unreadCount, markAllAsRead } = useNotifications();
  ...
  // WhatsApp Socket.io setup and event handlers
  useEffect(() => {
    socket.on("connect", () => {
    ...
    socket.on("ready", (msg) => {
      ...
      setTimeout(() => setShowWhatsAppQr(false), 3000); // auto-hide after connected
    });
          ...
          <div className="text-sm md:flex-grow md:flex md:items-center md:gap-2">
            ...
            <div className="relative">
              ...
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-6 w-6" // match notification icon size
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
              ...
              {showWhatsAppQr && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-lg z-50 border animate-fade-in-down">
                  ...
                  <div className="p-4 flex flex-col items-center">
                    <h4 className="text-center text-gray-800 font-semibold mb-3">
                      {whatsappStatus}
                    </h4>
                    {qr ? (
                      <QRCodeCanvas value={qr} size={256} />
                    ) : (
                      <p className="text-center text-gray-500">No QR code</p>
                    )}
                  </div>
                </div>
              )}
