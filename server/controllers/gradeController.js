...
//         // 🔒 ACCESS CONTROL
//         // ===============================
//         // 1. Admins can view but not modify
//         if (req.user.role === 'admin') {
//             return res.status(403).json({ message: "Admins cannot alter grade records." });
//         }

//         // 2. Teachers: only update their assigned subjects
//         if (req.user.role === 'teacher') {
//             const teacherSubjectIds = req.user.subjectsTaught.map(
//                 s => s.subject._id.toString()
//             );

//             if (!teacherSubjectIds.includes(grade.subject._id.toString())) {
//                 return res.status(403).json({ message: "Forbidden: You can only update grades for your assigned subjects." });
//             }
//         }
...
//             // Exclude the one who made the change
...
// --- Helper function for backward compatibility with Academic Year ---
...
exports.getGradeDetails = async (req, res) => {
    const { studentId, subjectId, semester, academicYear } = req.query;
    try {
        // 🌟 FIX: Apply Academic Year Flexibility logic
...
exports.updateGrade = async (req, res) => {
    try {
        ...
        if (req.user.role === 'teacher') {
            const teacherSubjectIds = req.user.subjectsTaught.map(s => s.subject._id.toString());
            if (!teacherSubjectIds.includes(grade.subject._id.toString())) {
                return res.status(403).json({ message: "Forbidden: You can only update grades for your assigned subjects." });
            }
        }
...
// @desc    Get a list of students and their scores for a specific assessment
// @route   GET /api/grades/sheet?assessmentTypeId=...&academicYear=...
exports.getGradeSheet = async (req, res) => {
    const { assessmentTypeId, academicYear } = req.query; // 🌟 FIX: Added academicYear extraction
    if (!assessmentTypeId) return res.status(400).json({ message: 'Assessment Type ID is required.' });
    ...
    try {
        ...
        // 🌟 FIX: Use the helper to support both new (2025-2026) and old (2025) data
        const yearQuery = getAcademicYearQuery(academicYear);

        // 🌟 FIX: Added academicYear to the database search query
        const query = { 
            student: { $in: studentIds },
            'assessments.assessmentType': assessmentTypeId
        };
...
exports.saveGradeSheet = async (req, res) => {
    ...
    try {
        // 🌟 FIX: Apply Academic Year Flexibility logic when finding existing document
        const yearQuery = getAcademicYearQuery(academicYear);
        ...
        for (const item of scores) {
            ...
            if (!gradeDoc) {
                // If not found, CREATE WITH THE NEW STANDARDIZED academicYear (e.g. 2025-2026)
                gradeDoc = new Grade({
                    student: item.studentId, 
                    subject: subjectId, 
                    semester, 
                ...
                // If found, update existing document but FORCE IT TO UPGRADE TO THE NEW FORMAT if it was old (e.g. 2025 -> 2025-2026)
