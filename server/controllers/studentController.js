...
// Generate short student ID: SchoolCode + Name Initials + YYMMDD
const generateStudentId = (schoolCode, fullName, dob) => {
  const names = fullName.trim().split(/\s+/);
  const initials = names.map(n => n[0].toUpperCase()).join("").slice(0, 2); // First 2 letters
  const date = new Date(dob);
  const dobString = date.toISOString().split("T")[0].slice(2).replace(/-/g, ""); // YYMMDD
  return `${schoolCode}-${initials}-${dobString}`;
};
...
exports.createStudent = async (req, res) => {
  ...
  try {
    const capitalizedFullName = capitalizeName(fullName);
    const studentId = generateStudentId("AKS", capitalizedFullName, dateOfBirth);
...
exports.updateStudent = async (req, res) => {
  try {
    const { fullName, ...otherData } = req.body;
    const updateData = { ...otherData };
    if (fullName) updateData.fullName = capitalizeName(fullName);
...
exports.uploadProfilePhoto = async (req, res) => {
  try {
    ...
    if (req.file.path && /^https?:\/\//i.test(req.file.path)) {
      ...
      const result = await cloudinary.uploader.upload(req.file.path, { folder: "student_photos" });
      student.imageUrl = result.secure_url;
      student.imagePublicId = result.public_id;
    ...
    await student.save({ validateBeforeSave: false });
    res.status(200).json({ success: true, message: "Profile photo updated successfully", imageUrl: student.imageUrl });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error uploading photo", details: error.message });
  }
};
...
exports.uploadReportCard = async (req, res) => {
  try {
    ...
    if (req.file.path && /^https?:\/\//i.test(req.file.path)) {
      ...
      const result = await cloudinary.uploader.upload(req.file.path, { folder: "report_cards" });
      student.reportCardUrl = result.secure_url;
      student.reportCardPublicId = result.public_id;
      try { if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path); } catch {}
    }
    ...
    await student.save({ validateBeforeSave: false });
    res.status(200).json({ success: true, message: "Report card uploaded successfully", reportCardUrl: student.reportCardUrl });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error uploading report card", details: error.message });
  }
};
...
exports.deleteReportCard = async (req, res) => {
  try {
    ...
    if (!student.reportCardPublicId) return res.status(400).json({ success: false, message: "No report card to delete." });

    try { await cloudinary.uploader.destroy(student.reportCardPublicId); } catch {}
    student.reportCardUrl = null;
    student.reportCardPublicId = null;
    ...
    res.status(500).json({ success: false, message: "Failed to delete report card." });
  }
};
...
exports.bulkCreateStudents = async (req, res) => {
  ...
  try {
    ...
    for (const studentRow of studentsJson) {
      ...
      let dob = studentRow["Date of Birth"] || studentRow["dateOfBirth"];
      if (typeof dob === "number") dob = new Date((dob - 25569) * 86400 * 1000);
      else if (typeof dob === "string") dob = new Date(dob);

      if (!(dob instanceof Date) || isNaN(dob.getTime())) {
        throw new Error(`Invalid date format for student: ${fullName}`);
      }

      const studentId = generateStudentId("AKS", capitalizedFullName, dob);
      const middleName = getMiddleName(capitalizedFullName);
      const yearOfBirth = dob.getFullYear();
      const initialPassword = `${middleName}@${yearOfBirth}`;
      ...
      const studentData = {
        ...
        password: initialPassword,
        parentContact: {
          parentName: studentRow["Parent Name"] || studentRow["parentName"],
          phone: studentRow["Parent Phone"] || studentRow["parentPhone"],
        },
        section: studentRow["Section"],
        rollNumber: studentRow["Roll No"] || studentRow["rollNumber"],
        motherName: studentRow["Mother's Name"] || studentRow["motherName"],
    ...
    if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    res.status(500).json({ success: false, message: "An error occurred during the import process.", details: error.message });
  }
};
...
exports.uploadClassTestReport = async (req, res) => {
  try {
    ...
    if (req.file.path && /^https?:\/\//i.test(req.file.path)) {
      ...
      const result = await cloudinary.uploader.upload(req.file.path, { folder: "class_test_reports" });
      secureUrl = result.secure_url;
      publicId = result.public_id;
      try { if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path); } catch {}
    }
    ...
    res.json({ success: true, message: "Class test report uploaded successfully", data: { url: secureUrl, public_id: publicId } });
  } catch (error) {
    res.status(500).json({ success: false, message: "Server error", details: error.message });
  }
};
...
exports.deleteClassTestReport = async (req, res) => {
  try {
    const student = await Student.findById(req.params.id);
    if (!student) return res.status(404).json({ success: false, message: "Student not found" });
    if (!student.reportClassTestPublicId) return res.status(400).json({ success: false, message: "No class test report to delete." });
