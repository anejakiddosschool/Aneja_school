...
// ================= PERIODIC TEST REPORT HANDLERS =================

// @route POST /api/students/:id/pt-report
exports.uploadPtReport = async (req, res) => {
  try {
    const studentId = req.params.id;
    ...
    student.reportPTUrl = secureUrl;
    student.reportPTPublicId = publicId;

    await student.save({ validateBeforeSave: false });

    return res.json({
      success: true,
      message: "Periodic Test report uploaded successfully",
      data: { url: secureUrl, public_id: publicId },
    });
  } catch (error) {
    console.error("Error uploading Periodic Test report:", error);
    res.status(500).json({
      success: false,
      message: "Server error",
      details: error.message,
...
// @route GET /api/students/:id/pt-report
exports.getPtReport = async (req, res) => {
  try {
    ...
    if (student.reportPTUrl) {
      return res.json({
        success: true,
        data: [
          {
            url: student.reportPTUrl,
            public_id: student.reportPTPublicId || "",
            uploadedAt: student.updatedAt || student.createdAt || new Date(),
          },
        ],
      });
    ...
    console.error("Error fetching Periodic Test report:", error);
    res.status(500).json({
      success: false,
      message: "Server error",
      details: error.message,
...
// @route DELETE /api/students/:id/pt-report
exports.deletePtReport = async (req, res) => {
  try {
    const student = await Student.findById(req.params.id);
    ...
    if (!student.reportPTPublicId) {
      return res.status(400).json({
        success: false,
        message: "No Periodic Test report to delete.",
      });
    }

    try {
      await cloudinary.uploader.destroy(student.reportPTPublicId);
    } catch (e) {
      console.warn("Cloudinary destroy failed:", e.message);
    }

    student.reportPTUrl = "";
    student.reportPTPublicId = "";
    await student.save();

    res.json({
      success: true,
      message: "Periodic Test report deleted successfully.",
    });
  } catch (error) {
    console.error("Error deleting Periodic Test report:", error);
    res.status(500).json({
      success: false,
      message: "Server error",
      details: error.message,
