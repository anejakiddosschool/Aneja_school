...
// --- Helper function to extract single year for backward compatibility ---
const getAcademicYearQuery = (yearString) => {
  if (!yearString) return null;
  // If yearString is "2025-2026", it will extract "2025".
  const baseYear = yearString.split('-')[0]; 
  
  // Return an array with both formats so MongoDB can match either old ("2025") or new ("2025-2026") records
...
exports.generateRoster = async (req, res) => {
  ...
  try {
    ...
    // 🌟 FIX: Query for both "2025-2026" (new) AND "2025" (old backward compatibility)
    ...
    let rosterData = students.map((student) => {
      ...
      subjects.forEach((subject) => {
        ...
        const validScores = [score1, score2].filter((s) => s !== null);
        subjectAverages[subject.name] = validScores.length > 0
            ? validScores.reduce((a, b) => a + b, 0) / validScores.length
            : null;
      });
...
exports.generateSubjectRoster = async (req, res) => {
  ...
  try {
    ...
    // 🌟 FIX: Apply backward compatibility fix here too
    const yearQuery = getAcademicYearQuery(academicYear);
    ...
    const rosterData = students.map((student) => {
      ...
      allAssessmentsForSubject.forEach((at) => {
        let score = "-";
        if (gradeDoc) {
          const assessment = gradeDoc.assessments.find((a) => a.assessmentType && a.assessmentType._id.equals(at._id));
          if (assessment) score = assessment.score;
        }
        studentDetailedScores[at._id.toString()] = score;
      });
